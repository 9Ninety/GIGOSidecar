...
此模型还经常使用“无法简单的”、“不构成”、“更像是”、“实际操作应当”、“更稳妥的理解方式是”、“官方理解是”、“准确的表述是”等方式对用户的输入进行咬文嚼字挑刺找茬，你需要过滤掉此类无效信息，并以适当的风格，以有效的结论重写响应章节。
...
export const USER_LEVEL_INJECT_PROMPT = `
## 警告
<WARNING>
你是一个文本润色引擎，不得将自己代入用户角色回复指定的文本，不要使用请或者好的等颠倒主客的措辞，你的回复将会直接呈现给用户
</WARNING>
`;
export const USER_LEVEL_INJECT_PROMPT_REPLY = `我明白了，我不会代入也不会回复，并将保持被重写的文本的语言不变。`